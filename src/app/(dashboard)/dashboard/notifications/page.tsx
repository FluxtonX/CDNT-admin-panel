...
function TypeBadge({ type }: { type: NotificationType }) {
  const map: Record<NotificationType, { cls: string; icon: React.ReactNode }> = {
    Info: { cls: "bg-blue-50 text-blue-700 border-blue-200", icon: <Info className="h-3 w-3" /> },
...
function UserPickerModal({
  ...
  }, [open]); // intentionally not including initialSelected to avoid reset mid-session
        ...
        >
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
            <div className="flex items-center gap-3">
              <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-blue-50 text-[#0A3D91]">
              ...
              {(["All", "Verified", "Unverified"] as FilterTab[]).map((tab) => (
                ...
                >
                  ...
                  <span className={cn(
                    ...
                    filterTab === tab ? "bg-blue-100 text-[#0A3D91]" : "bg-gray-200 text-gray-500"
                  )}>
                    {tabCounts[tab]}
                  </span>
                </button>
          ...
          <div className="px-6 pt-3">
            <label className="flex items-center gap-2.5 h-10 rounded-xl border border-gray-200 bg-gray-50 px-3 focus-within:border-blue-400 focus-within:ring-2 focus-within:ring-blue-50 transition-all">
          ...
          <div className="flex items-center justify-between px-6 pt-2.5 pb-1">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="text-xs font-bold text-[#0A3D91] hover:text-blue-800 disabled:opacity-40 disabled:cursor-not-allowed transition-colors cursor-pointer"
              >
                ...
                return (
                  ...
                  >
                    <input
                      ...
                      className="h-4 w-4 rounded border-gray-300 accent-[#0A3D91] shrink-0 cursor-pointer"
                    />
                    <div className="min-w-0 flex-1">
                      <p className={cn("text-sm font-semibold truncate", checked ? "text-[#0A3D91]" : "text-gray-900")}>
                        {user.name}
                      </p>
                    ...
                    {checked && <Check className="h-3.5 w-3.5 text-[#0A3D91] shrink-0" />}
                  </label>
                );
              })
            )}
          ...
          <div className="px-6 py-4 border-t border-gray-100 bg-gray-50/60 flex items-center justify-between gap-4">
            <div className="flex items-center gap-2">
              <div className={cn(
                ...
                selectedCount > 0 ? "bg-[#0A3D91] text-white" : "bg-gray-200 text-gray-500"
              )}>
                {selectedCount}
              </div>
...
export default function NotificationCenterPage() {
  ...
  /* ── Stats cards ── */
  const stats = [
    { label: "Total Users", value: totalUsers.toLocaleString(), icon: <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-blue-50 text-[#0A3D91]"><Users className="h-5 w-5" /></div> },
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm space-y-5">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-0.5">Configure and send to specific recipients</p>
            </div>
                ...
                {(["Info", "Warning", "Success", "Error"] as NotificationType[]).map((t) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        "flex items-center justify-center gap-1.5 py-2 px-2 border rounded-xl text-xs font-bold transition-all cursor-pointer",
                        active ? "bg-blue-50 text-[#0A3D91] border-blue-300" : "bg-gray-50 text-gray-600 border-gray-200 hover:bg-gray-100"
                      )}
                    >
                      <span className={cn(
                        "h-1.5 w-1.5 rounded-full shrink-0",
                        t === "Info" ? "bg-blue-500" : t === "Warning" ? "bg-amber-500" : t === "Success" ? "bg-green-500" : "bg-red-500"
                      )} />
                      {t}
                    </button>
                  );
              ...
              <button
                ...
                className={cn(
                  ...
                  confirmedCount > 0
                    ...
                    : "border-dashed border-gray-300 bg-gray-50 text-gray-500 hover:border-blue-300 hover:bg-blue-50/40 hover:text-[#0A3D91]"
                )}
              >
            ...
            <div className="space-y-2 pt-1">
              <label className="text-[10px] font-extrabold text-gray-500 uppercase tracking-widest block">Send Notification</label>
              <div className="grid grid-cols-3 gap-2">
                ...
                <button
                  ...
                  className={cn(
                    ...
                    confirmedCount > 0 && !sending
                      ? "border-[#0A3D91] text-[#0A3D91] bg-white hover:bg-blue-50 active:scale-[0.98]"
                ...
                <button
                  ...
                  className={cn(
                    "flex flex-col items-center justify-center gap-1.5 py-3 px-2 rounded-xl border-2 text-xs font-bold transition-all cursor-pointer",
                    confirmedCount > 0 && !sending
                      ? "border-violet-400 text-violet-700 bg-violet-50 hover:bg-violet-100 active:scale-[0.98]"
                      : "border-gray-200 text-gray-400 bg-gray-50 cursor-not-allowed opacity-60"
                  )}
                >
                  {sending === "both"
                    ? <span className={cn(spinnerCls, "border-violet-500")} />
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm flex flex-col min-h-[460px]">
            <div className="mb-4">
              ...
              <p className="text-xs text-gray-500 mt-0.5">Audit log of dispatched notifications</p>
            </div>
            ...
            {loading ? (
              ...
              <div className="flex-1 overflow-y-auto space-y-3 pr-1">
                {notifications.map((notif) => (
                  <div key={notif.id} className="border border-gray-100 bg-white rounded-2xl p-4 space-y-2.5 shadow-sm hover:border-gray-200 transition-colors">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div className="flex items-center gap-2">
                        <TypeBadge type={notif.type} />
                        <span className="font-extrabold text-gray-900 font-mono text-xs">{notif.id}</span>
                      </div>
                    ...
                    <div className="space-y-0.5">
                      <h4 className="text-sm font-extrabold text-gray-900">{notif.title}</h4>
                      <p className="text-xs text-gray-600 leading-relaxed line-clamp-2">{notif.message}</p>
                    </div>
                    <div className="pt-1.5 border-t border-gray-100 text-[10px] font-bold text-gray-500 uppercase tracking-wide">
                      {notif.audienceLabel}
                    </div>
                  </div>
                ))}
